...
.intro-section { background: rgba(30,41,59,0.85); backdrop-filter: blur(12px); border-radius: 20px; padding: 20px; margin-top:20px; box-shadow:0 8px 25px rgba(0,0,0,0.3); color:#e2e8f0; position: relative; z-index: 1; }
...
.vuln-card .accordion-button:not(.collapsed) {
    color: #38bdf8 !important;
    background-color: transparent !important;
}
.vuln-card .accordion-button::after {
    filter: brightness(0) invert(1);
    flex-shrink: 0;
    margin-left: 1rem;
}
.vuln-card .accordion-button:focus {
    box-shadow: none;
...
<div class="intro-section animate__animated animate__fadeIn">
    <div class="title-container d-flex align-items-center justify-content-center flex-wrap mb-2">
        <h2>Welcome to Web Scanner</h2>
        <?php if(!isset($_SESSION['username'])): ?>
            <p class="text-light small mb-0 ms-3">
                (<a href="register.php" class="source-link">Register</a> or <a href="login.php" class="source-link">Login</a> to start)
            </p>
        <?php endif; ?>
    </div>
